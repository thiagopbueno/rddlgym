...
class ReservoirBuilder(RDDLBuilder):
    ...
    REWARD = """sum_{?r: res} [
        LOW_PENALTY(?r) * max[0.0, LOWER_BOUND(?r) - rlevel(?r)]
        + HIGH_PENALTY(?r) * max[0.0, rlevel(?r) - UPPER_BOUND(?r)]
        + SET_POINT_PENALTY(?r) * abs[(LOWER_BOUND(?r) + UPPER_BOUND(?r)) / 2 - rlevel(?r)]
    ];"""
